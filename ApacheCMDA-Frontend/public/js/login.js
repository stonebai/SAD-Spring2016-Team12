...
function passwordCheck () {
    var stage = $("#logForm").attr("data-target")
    var input_username = $("#inputUname").val();
    var input_passwd = $("#inputPassword").val();
    if ( input_username.length == 0 || input_passwd.length == 0) {
        showWarning("Input cannot be empty");
        return false;
    }
    if (stage == "signup") {
        var input_passwd2 = $("#inputPassword2").val();
        if (input_passwd != input_passwd2) {
            showWarning("Two passwords don't match");
            return false;
        }
        if (input_passwd.length <= 3) {
            showWarning("Password too short");
            return false;
        }
        if (checkReservedWords(input_username)) {
            return true;
        } else{
            showWarning("Reserved Word Used");
            return false;
        }
...
$(function() {
    var warnning = $(".warning p");
    $(warnning).addClass("bg-danger");
    $(warnning).removeClass("bg-danger");
    $(warnning).addClass("bg-warning")
})
